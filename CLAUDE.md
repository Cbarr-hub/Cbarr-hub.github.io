...
backend/                Fastify backend
  src/
    routes/             API route handlers (servers.js = OPS-table dispatcher)
    servers/            game server control (service, engine, registry, session-sql)
      connectors/engine.js   ONE GameConnector interpreting per-game specs
      connectors/specs/      declarative per-game specs (data + imperative hooks)
    docker/             Docker API client (game-server control transport)
    economy.js          playtime → gambling-dollars reconciler (migration 007)
...
servers.compose.yml     the 7 game-server containers + docker-proxy + BlueMap map
docker/                 game-server images (gmod/) + BlueMap config (bluemap/config)
...
# Restart / logs (services: app, caddy, minecraft, rlcraft, gmod, prophunt, counterstrike, factorio, valheim)
$COMPOSE restart app
...
- **Startup-config "Profiles" (servers panel):** named structured startup configs per server, stored in SQLite (`server_profiles` + `server_active_profile`, migration 003). A profile is what the server *boots as*; live RCON/console commands are ephemeral and never written back. The engine (`connectors/engine.js`) owns the generic lifecycle (list/get/create/update/delete/apply/capture + an auto-seeded "Default"); each game's spec (`connectors/specs/<game>.js`) supplies `profile.{defaults,validate,schema,apply,capture}`. **All seven games are wired for profiles.** For GMOD the rotation's **first map is the boot map** (no separate field), and the panel's **Apply = apply config + restart** so the collection actually mounts. The panel shows a **Profiles** tab (config) AND a **Quick Settings** surface (operations like Factorio Save As) — a game can have one or both; a game wired for profiles trims its `getSettings` so config doesn't double-render. Saved profile field keys/bounds are pinned by `backend/test/goldens.mjs` (the per-game canon any connector change must reproduce).
- **Servers panel power = the container model.** `servers.html` is a **fleet deck** (per-game tiles with live CPU/RAM + a fleet total) over a per-game management sheet with tabs **Overview · Runtime · Profiles · Maps · Config** (the old Connect/Tweak/Full mode switch is gone; "Basics only" on Profiles covers Tweak). Power controls are container-level — **Start · Restart · Stop · Force Stop · Update · Refresh** — with **no** separate "Game Service" buttons (the container *is* the game; the legacy `startGame`/`stopGame`/`restartGame` action names are still accepted and **alias to container power in `service.doAction`** — never `BAD_ACTION`). **Update the game client is in-panel for all five games** (shown only while the container is running): update recipes live ONCE in each game's spec (`spec.update`) — GMOD/PH run LinuxGSM `./gmodserver update`, CS runs SteamCMD `+app_update 730`, Minecraft/Factorio restart so the image re-resolves its version (a true Factorio bump still needs a host `docker compose pull`); `tools/gt-maintenance.mjs` runs the SAME spec recipes for idle auto-updates. **Apply & Restart** reboots the container (which also remounts a GMOD workshop collection). **Backups aren't an app feature at all** — the app holds no R2 keys; production backups run from the **host systemd timer** only (see [`docs/disaster-recovery.md`](docs/disaster-recovery.md)). Container power + update model codified in `backend/test/docker.test.mjs` + `backend/test/goldens.mjs`.
- **Runtime panel: binary toggles are buttons, ranges are sliders.** `getLive()` returns `actions` (binary toggles / instant commands → buttons) AND `controls` (continuous cvars → sliders, pushed via `runLiveAction(key, value)`). Both are spec data rows the engine dispatches: each control row carries bounds + a `cmd(n)` builder; GMOD-family rows are `strict` (non-numeric input throws) while Factorio/MC/PH-timer rows soft-clamp (empty → default, 0 is a real value). The shared GMOD gravity/timescale rows live in `specs/gmod.js` and are composed by `specs/prophunt.js`. Exact command strings + clamp samples are golden-pinned.
- **CS settings (counterstrike): live-apply, collection import, auto names.** CS is the one game whose profile **Apply pushes LIVE over RCON, NOT by restart** (a restart reverts to the compose env) — `specs/counterstrike.js`'s schema returns an `apply: { mode:'live', label, confirm, note }` descriptor the panel reads to relabel the button (**▶ Apply Live**) and **skip the reboot**; the apply itself is ordered (map change LAST) and batched ≤1800 chars per RCON command. `maxPlayers` is **not** a CS profile field (env-only / `CS2_MAXPLAYERS`; Apply can't change it). The workshop-map catalog supports **collection import** (`POST /:id/maps/collection`) and **auto-fetched names** (omit `name` on `POST /:id/maps`) via the **keyless** Steam endpoints in `backend/src/servers/steam-workshop.js` (an optional `STEAM_API_KEY` is passed through if set but never required).
- **Prop Hunt = the `prophunt` container, port 27067.** Same `gamertown-gmod` image as TTT (built from `docker/gmod/Dockerfile`), but `GAMEMODE=prop_hunt` with its **own** `PROPHUNT_GSLT` so it runs alongside TTT. `specs/prophunt.js` composes the shared GMOD-family pieces exported by `specs/gmod.js` (the `/data` LinuxGSM layout, gmad map sync with `mapPrefixes=['ph_','gm_']`, shared sliders/commands; RCON on the registry game port). **Workshop content mounts from the public Steam collection `3737190377`** (`wscollectionid` / `PROPHUNT_WORKSHOP_COLLECTION`) — GMOD downloads + mounts it at boot. The **Prop Hunt: X2Z** gamemode addon ships the `prop_hunt` (+ `base_phx`) folder, so `gamemode=prop_hunt` loads; the 7 `ph_` maps + taunt packs + loadout manager come along, and clients auto-download via the collection. PH's profile apply deliberately does **NOT** write `wscollectionid` (so Apply can't break the mount). Edit X2Z config via the panel's **Config** tab: `server.cfg`/`active.cfg` for `ph_`/`phx_` cvars, `lgsm.cfg` for the collection id/ports, plus X2Z's `phx-loadout`/`phx-admins` data files. NOTE: `prop_hunt` is not built into GMOD (only base/sandbox/terrortown ship), so the collection mount is required to boot PH. **`3736674438` is a working TTT *maps* collection** (separate from PH) — verified 2026-06-04 to mount 5 `ttt_` maps (ttt_clue_se, ttt_diescraper, ttt_dolls, ttt_minecraft_b5, ttt_waterworld).
...
- **Offsite backups → Cloudflare R2 (`gamertown-backups`, remote `r2`) are LIVE — host-driven.** A **host systemd timer** (`gt-db-backup.timer`, **daily 04:00** → `/usr/local/bin/gt-backup.sh`; vendored at [`tools/gt-backup.sh`](tools/gt-backup.sh) + [`tools/systemd/`](tools/systemd/)) pushes the **app DB** (`app/`, keep 14), the **Factorio active save** (`factorio/`, keep 14), and the **Minecraft world** (`minecraft/`, ~5 GB gz, keep 1 — flushed via the container's `rcon-cli` for a consistent snapshot; the ~5 GB world dominates a ~10 GB R2 budget, so only the latest is kept). The **age-encrypted secret bundle** (`secrets/`) is on-demand (`tools/secrets-backup.sh`). rclone runs as host **root** (keys in `/root/.config/rclone/rclone.conf`); the repo/app/**containers never hold R2 keys** — so backups are the host timer's job, **not** an in-panel feature (the app reaches the engine only through the scoped socket-proxy, with no path to R2). **rclone must be ≥ ~1.66 (we run 1.74.2)** — stock **1.60.1 returns `501 NotImplemented` on R2 `rcat`**. Prune keeps the newest N by the `_YYYYMMDD_HHMMSS` **in the name**, NOT object ModTime. Full backup map + recovery → [`docs/disaster-recovery.md`](docs/disaster-recovery.md).
- **Playtime ECONOMY + presence/Activity + embedded MAP (app-side, 2026-06-07).** Three features layered on top of the host session-tracker's `players`/`server_sessions` rows: (1) **Economy** (migration 007 → `app_settings`, `player_accounts`, `server_sessions.credited_at`; `backend/src/economy.js` as `app.economy`; routes under `/api/admin/economy/*`) — an in-process reconciler credits each CLOSED, uncredited, LINKED game-server session to a site balance **once** (idempotent via `credited_at`; per-session cap bounds the award), running at boot + on a **5-min timer**; an admin **Economy** view sets the `$/hour` rate + cap and links a tracked identity (`players.id`) to a site account (only post-link playtime earns — linking settles prior sessions). (2) **Presence + Activity** (`GET /api/servers/online` = open sessions, `GET /api/servers/activity?limit=1..500` = recent join/leave timeline) — read-only, rendered as the panel's standalone **Activity** view. (3) **Map** (Minecraft-only) — the `bluemap` container (in `servers.compose.yml`) renders a 3D Minecraft world map from a **read-only** world mount, served on `:8100` and reverse-proxied by Caddy at **`/map/*`** behind the `forward_auth` login gate (`GET /api/auth/gate`); `servers.html` embeds it full-bleed in a **Map** tab (overlay toolbar + clickable live-player list + **Detail radius** LOD slider). **Live player markers (with real skins):** since the renderer runs standalone (no server connection), `backend/src/servers/bluemap.js` (`createBlueMapPlayersController`, wired in `server.js`; 2s poll backing off to 10s when nobody's online) polls **live online players directly over RCON** (`serverService.liveOnlineWithPositions`; names→Mojang UUID via the server's `usercache.json` — 60s-TTL-cached in `specs/minecraft.js` — NOT the host session-tracker, so markers render in dev too) and writes BlueMap's native `web/maps/<id>/live/players.json` + per-UUID head PNGs under **each map's own asset root** `web/maps/<id>/assets/playerheads/<uuid>.png` (the path BlueMap v5 actually loads heads from — NOT a global `web/assets/`; skins from `BLUEMAP_SKIN_BASE`/mc-heads) into the container via the docker-proxy `EXEC` path — markers are BlueMap's head billboards, not 3D bodies; needs `MINECRAFT_RCON_PASSWORD`. The same module owns the **BlueMap CPU tuner** (app-side ONLY; presence from a cheap open-session COUNT). **No Factorio map** — the headless Factorio image can't screenshot, so no embeddable renderer exists (the old placeholder "Factorio mode" was removed). BlueMap config is bind-mounted from `docker/bluemap/config`; `webapp.conf` and `storages/file.conf` are tracked for deterministic 3D LOD/storage defaults, while generated `packs/` stays ignored. Preserve the `bluemap-data` + `bluemap-web` volumes so restarts use BlueMap's changed-region update path instead of full rebuilds; details: [`docs/bluemap-performance.md`](docs/bluemap-performance.md). The servers panel's nav peers are now **Activity · Map · Economy** (plus the **Data** DB viewer). See [`docs/backend.md`](docs/backend.md).
- **Player-session tracking is HOST-side, not the app.** A long-running host **systemd service** (`gt-session-tracker.service` → [`tools/gt-session-tracker.mjs`](tools/gt-session-tracker.mjs), vendored with [`tools/systemd/`](tools/systemd/)) records who joins/leaves each server, **independent of the website** (the app only *reads* the rows, for the servers panel's **standalone "Activity" section** — NOT a per-game tab; the playtime economy reconciler consumes the same rows). **Hybrid collection:** it **tails `docker logs -t`** for **Minecraft + Factorio** (accurate join/leave times; Minecraft also yields the Mojang UUID) and **RCON-polls `status` every 60s** for the **Source games** (GMOD/Prop Hunt/CS2 — ±60s; GMOD/PH give a SteamID64, **CS2 is name-only** because its `status` redacts SteamIDs). It's pure Node (imports the dep-free `registry.js` + `rcon-tcp.js` + `connectors/online-parse.js` + `session-sql.js`) and writes via the host `sqlite3` CLI — so it needs **`node` + `sqlite3` on the keeper** and **full `docker` access** (the app can't, via the scoped socket-proxy). **Schema (migration 005):** sessions live in `server_sessions` + a global `players` roster (the cross-game whitelist seed: one SteamID64 spans the 3 Source games; null-uid CS2 rows get no `players` row); the 7 game *servers* are rows in the **existing `games` table tagged `hosted=1`** (seeded from the registry on app boot), so `/api/games` filters `hosted=0` to keep the party-games picker clean. The session-write SQL lives ONCE in **`backend/src/servers/session-sql.js`** — `store.js` prepares it and the collector renders the same strings to sqlite3-CLI literals (no hand-mirrored copies). **Validated live in the dev stack (2026-06-06):** CS2's `status` is a name-only **`---players---` block** (single-quoted names, *redacted* SteamID, `BOT` in the time column, empty `''` connecting slots) — nothing like legacy srcds, so `parseSourceStatus` **dispatches** a CS2 branch vs the GMOD/PH `#userid "name" STEAM_…` rows. The collector sets the sqlite busy timeout with the **`.timeout` dot-command, NOT `PRAGMA busy_timeout=N;`** (that pragma emits a value row that pollutes `-json` reads → phantom `{timeout}` rows / broken `JSON.parse`), and needs host **`sqlite3` ≥ 3.38** (`unixepoch()` + `-json`; probed at startup, fail-fast). Minecraft (UUID) + Factorio (`[JOIN]` on stdout) log formats confirmed against the real containers.
- **Valheim = the `valheim` container, UDP 2456–2457, and it has NO RCON.** Image `ghcr.io/community-valheim-tools/valheim-server` (lloesche): steamcmd install on first boot, its OWN idle-gated auto-update + hourly world zips (`/config/backups`), supervisord. Consequences in the panel: `specs/valheim.js` has **no `rcon`/`live` block** → the server row carries `capabilities.live=false` and `servers.html` **hides the Runtime tab**; `sendCommand` → `NO_RCON`; profile startup commands skip. **Presence is host-tracker-only** (`collect:'log'`, `identityKind:'steam'` → `parseValheimLog` in `online-parse.js`): a join is `Got handshake from client <SteamID64>` → `Got character ZDOID from <Name> : <peer>:<n>` (a `0:0` ZDOID is a DEATH, not a join), a leave is `Closing socket <SteamID64>`; the tracker pairs uid→name FIFO. No live overlay is possible (the image's A2S `/status.json` reports EMPTY player names). **Profiles = an env-override file**: Apply writes `/config/gamertown.env` (`SERVER_NAME`/`WORLD_NAME`/`SERVER_PASS`/`SERVER_ARGS` with `-preset` → `-modifier` → `-setkey` → `-saveinterval`/`-backups`, single-quoted) and the compose `PRE_SERVER_RUN_HOOK` sources it in the launcher's own shell before the command line is built, so it overrides the env on every restart; an EMPTY profile field = inherit compose. **Keep `SERVER_PUBLIC=1` + password** (the image's idle detection is an A2S query that only answers on public servers — update/backup crons are gated on it) and **`CROSSPLAY=false`** (PlayFab ids would break the SteamID roster and need UDP 2458). In-panel **Update = `supervisorctl signal HUP valheim-updater`** (the image checks Steam now and restarts only the game if files changed); `gt-maintenance` pulls the wrapper image. Join string `host:2456 (password: …)`, launch `steam://run/892970//+connect host:2456 +password …`. Secrets: `VALHEIM_SERVER_PASS` in the **project `.env` only** (compose `:?` fails fast without it) — the app reads it back via `printenv` in-container (60 s cache). R2: `gt-backup.sh valheim` ships the newest in-volume zip (keep 14).
- **Host maintenance (`gt-maintenance.service`)** runs [`tools/gt-maintenance.mjs`](tools/gt-maintenance.mjs) on the keeper with full Docker/Compose access: hourly game-update checks (compose pulls for MC/Factorio + the spec update recipes for the Steam games), gated on the `server_sessions` open-row count so they run only when nobody is online. **BlueMap CPU tuning moved app-side** (`backend/src/servers/bluemap.js`, through the socket proxy) — the maintenance daemon no longer tunes it; image pulls/recreates stay host-side.
