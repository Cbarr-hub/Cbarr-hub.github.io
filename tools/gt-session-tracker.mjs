...
// "Events" tab. Host root has full `docker logs`/`docker inspect` + direct DB-volume
...
function startLogTail(server) {
  ...
  const open = new Map();        // name → session id
  ...
  const handleEvent = async (ev, ts) => {
    ...
    if (ev.kind === 'join') {
      ...
      await recordJoin(slug, { name: ev.name, uid, identityKind }, ts, 'log');
      // recordJoin via the CLI doesn't return the id; re-read the open session for this name.
      const rows = await openSessionsFor(slug);
      const mine = rows.find((r) => r.name === ev.name);
      if (mine) open.set(ev.name, mine.id);
      return;
    }
    if (ev.kind === 'leave') {
...
async function pollRcon(server) {
  ...
  const ts = nowSec();
  const live = new Map(roster.map((p) => [p.uid ?? `name:${p.name}`, p]));
  const openRows = await openSessionsFor(slug);
  const openByKey = new Map(openRows.map((r) => [r.uid ?? `name:${r.name}`, r]));
