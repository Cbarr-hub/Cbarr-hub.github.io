...
$projenv_path = Join-Path $secrets_dir "root" | Join-Path -ChildPath "gamertown" | Join-Path -ChildPath ".env"
if (-not (Test-Path $projenv_path)) {
    Write-Err "Expected project env not found after extraction: $projenv_path"
    Write-Host "  The dev stack needs it for `${...} interpolation (game RCON/GSLT). Bundle layout may differ."
    exit 1
}
foreach ($k in 'MINECRAFT_RCON_PASSWORD','CS2_RCON_PASSWORD','GMOD_RCON_PASSWORD','PROPHUNT_RCON_PASSWORD') {
    if (-not (Select-String -LiteralPath $secrets_env_path -Pattern "^$k=.+" -Quiet)) {
        Write-Host "[!] $k missing/empty in secrets.env - game servers may fail to start" -ForegroundColor Yellow
    }
}

# ── upsert .env.local (forward slashes, no BOM, preserve unknown keys) ──────────
# docker compose --env-file is happier with forward slashes on Windows. Only
# GT_SECRETS_FILE / GT_CERTS_DIR are managed here; anything else the user added
# (e.g. DEV_PUBLIC_HOST for joining dev game servers) is carried forward, so
...
$managed = @(
    "# Generated by tools/setup.ps1 (managed keys: GT_SECRETS_FILE, GT_CERTS_DIR)",
    "GT_SECRETS_FILE=$secrets_env_fwd",
    "GT_CERTS_DIR=$certs_dir_fwd"
)
$preserved = @()
if (Test-Path $env_file) {
    $preserved = Get-Content -LiteralPath $env_file | Where-Object {
        $_ -notmatch '^\s*#' -and $_ -notmatch '^\s*GT_SECRETS_FILE=' -and $_ -notmatch '^\s*GT_CERTS_DIR='
    }
}
Write-TextNoBom -Path $env_file -Content ((($managed + $preserved) -join "`n") + "`n")
Write-Success "Wrote $env_file"
Write-Host ""
