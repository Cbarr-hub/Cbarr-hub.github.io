...
- **Host maintenance (`gt-maintenance.service`)** runs [`tools/gt-maintenance.mjs`](tools/gt-maintenance.mjs) on the keeper with full Docker/Compose access. It uses the same `server_sessions` open-row count as the panel: BlueMap is CPU-capped low while any hosted server has players online, then ramps high after an idle delay; hourly game-update checks run only when nobody is online. The app also has a lightweight BlueMap CPU tuner through the socket proxy, but image pulls/recreates stay host-side.
